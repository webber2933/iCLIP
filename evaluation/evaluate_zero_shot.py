...
def frameAP(groundtruth, detections, thr, print_info=True):
    ...
    results = {}
    train_label_file = open("/home/deeperAction22/ActionRec/webber/jhmdb_label_split/75vs25/1/train_label.txt", "r")
    data = train_label_file.read()
    our_train_labels = data.split("\n")
    our_train_labels = our_train_labels[:-1]
    train_label_file.close()
    
    for ilabel, label in enumerate(GT['labels']):
        # detections of this class
        if label in our_train_labels:
           if print_info:
