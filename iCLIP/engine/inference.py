...
from iCLIP.dataset.datasets.evaluation import evaluate
from iCLIP.utils.comm import get_rank, is_main_process, all_gather, gather, synchronize, get_world_size
from iCLIP.structures.memory_pool import MemoryPool
...
def compute_on_dataset_2stage(model, data_loader, device, logger):
    ...
    for i, batch in enumerate(tqdm(data_loader, **extra_args)):
        ...
        for j, (movie_id, timestamp, p_ft, o_ft, )in enumerate(zip(movie_ids, timestamps, person_feature, object_feature)):
            # p_ft.extra_fields['keypoints'] = boxes[j].extra_fields['keypoints']
            # p_ft.extra_fields['keypoints_score'] = boxes[j].extra_fields['keypoints_score']
            person_feature_pool[movie_id, timestamp] = p_ft
        # store other information in list, for further inference
        batch_info_list[i] = (movie_ids, timestamps, video_ids, object_feature, keypoints_feature, poses_feature, [b.extra_fields['det_score'] for b in boxes])
    ...
    start_time = time.time()
    for movie_ids, timestamps, video_ids, object_feature, keypoints_feature, poses_feature, det_scores in tqdm(batch_info_list, **extra_args):
...
def _accumulate_predictions_from_multiple_gpus(predictions_per_gpu):
    ...
    if len(video_ids) != video_ids[-1] + 1:
        logger = logging.getLogger("iCLIP.inference")
        logger.warning(
