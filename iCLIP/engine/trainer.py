...
from iCLIP.utils.metric_logger import MetricLogger
from iCLIP.engine.inference import inference
from iCLIP.utils.comm import synchronize, reduce_dict, all_gather
...
def do_train(
    ...
    logger = logging.getLogger("iCLIP.trainer")
    ...
    for iteration, (slow_video, fast_video, boxes, objects, keypoints, extras, _) in enumerate(data_loader, start_iter):
        ...
        # webber: save the model with min loss
        if losses < min_loss:
            min_loss = losses
