...
class Transformer(nn.Module):
    def __init__(self, width: int, layers: int, heads: int, attn_mask: torch.Tensor = None, droppath=None, use_checkpoint=False, T=1):
        super().__init__()
...
class CrossFrameCommunicationTransformer(nn.Module):
    def __init__(self, input_resolution: int, patch_size: int, width: int, layers: int, heads: int, output_dim: int,
        ...
        self.conv1 = nn.Conv2d(in_channels=3, out_channels=width, kernel_size=(patch_size, patch_size), stride=(patch_size, patch_size), bias=False)

        scale = width ** -0.5
        self.class_embedding = nn.Parameter(scale * torch.randn(width))
        self.positional_embedding = nn.Parameter(scale * torch.randn((input_resolution // (patch_size*patch_size)) + 1, width))
        self.ln_pre = LayerNorm(width)

        # Attention Blocks
        self.transformer = Transformer(width, layers, heads, use_checkpoint=None, T=T,)
