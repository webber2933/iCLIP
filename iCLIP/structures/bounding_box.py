...
class BoxList(object):
    ...
    def top_k(self, k, boxes):
        ...
        #     # return bbox
        # # categories, bbox, scores
        # # Find a sweet spot between iou and score
        # # Error here
        # elif score_plus_iou == True:
        ...
        #     else:
        #         # 
        #         idx_iou = np.where(iou > 0)[0]
        #         idx_iou = torch.tensor(idx_iou)
        #         bbox = BoxList(self.bbox[[idx_iou]], self.size, self.mode)
        #         for k, v in self.extra_fields.items():
        #             if isinstance(v, torch.Tensor):
        #                 bbox.add_field(k, v[:k])
        #             else:
