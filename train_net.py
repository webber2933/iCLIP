...
from iCLIP.solver import make_lr_scheduler, make_optimizer
from iCLIP.engine.inference import inference
from iCLIP.engine.trainer import do_train
...
from iCLIP.utils.logger import setup_logger, setup_tblogger
from iCLIP.utils.random_seed import set_seed
...
def train(cfg, local_rank, distributed, tblogger=None, transfer_weight=False, adjust_lr=False, skip_val=False,
    ...
    actionlist, actiondict, actiontoken = text_prompt(dateset_label=cfg.DATASET_LABEL, clipbackbone='ViT-B/16', device=device)
...
def main():
    ...
    num_gpus = int(os.environ["WORLD_SIZE"]) if "WORLD_SIZE" in os.environ else 1 # webber: we use 1 gpu
    ...
    logger = setup_logger("iCLIP", output_dir, global_rank)
